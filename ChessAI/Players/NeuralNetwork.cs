...
namespace ChessAI
{
    class NeuralNetwork : Player
    {
        ...
        public NeuralNetwork Learn(int games)
        {
        ...
        public override Point[] Decide(Board board)
        {
            ...
            do
            {
                choice = DoubleToMove(network.Compute(board.BoardToDouble())[0], eps);
                toMove = board.BoardTab[choice[0].x, choice[0].y];
                cnt++;
            }
            while ((toMove == null || toMove.Color != color || toMove.GetValidMoves(board, choice[0]).Where(x => x.x == choice[1].x && x.y == choice[1].y).Count() == 0) && cnt < 5);
